...
class MyModel(QAbstractTableModel):
    ...
    def data(self, index, role):
        if role == Qt.DisplayRole:
            return DATA[self._data[index.row()]]  # return a string (for the view)
        if role == Qt.EditRole:
            return self._data[index.row()]        # return an integer (for the delegates editor widget)
        return QVariant()
...
class MyDelegate(QStyledItemDelegate):
    ...
    def setEditorData(self, editor, index):
        ...
        """
        value = index.data(Qt.EditRole)       # equivalent of    value = index.model().data(index, Qt.EditRole)      # <<< TODO: display role or editor role ?

...
# Usage example:
# ./getting_started_time_series_forecasting_transformers_args.py --train-csv=getting_started_time_series_forecasting_dense_layers_args.csv --test-csv=getting_started_time_series_forecasting_dense_layers_args.csv --train-column=0 --test-column=1 --train-skip=1 --test-skip=1 --seq-len=48 --pred-len=24 --plot
...
class TimeSeriesModel(torch.nn.Module):
    def __init__(self, input_dim: int, output_dim: int, num_heads: int, num_layers: int) -> None:
...
def main() -> None:
    ...
    task = Task.init(project_name="Snippets", task_name="Timeseries forecasting with Transformers")
    ...
    model = TimeSeriesModel(input_dim=args.seq_len, output_dim=args.pred_len, num_heads=8, num_layers=2).to(args.device)
    ...
    torch.save(model, "getting_started_time_series_forecasting_transformers.pth")

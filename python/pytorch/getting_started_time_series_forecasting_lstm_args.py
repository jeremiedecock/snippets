...
# Usage example:
# ./getting_started_time_series_forecasting_lstm_args.py --train-csv=getting_started_time_series_forecasting_dense_layers_args.csv --test-csv=getting_started_time_series_forecasting_dense_layers_args.csv --train-column=0 --test-column=1 --train-skip=1 --test-skip=1 --seq-len=48 --pred-len=24 --plot
...
def main() -> None:
    ...
    task = Task.init(project_name="Snippets", task_name="Timeseries forecasting with LSTM")
    ...
    model = TimeSeriesModel(input_dim=1, hidden_dim=64, output_dim=args.pred_len, num_layers=2).to(args.device)
    ...
    torch.save(model, "getting_started_time_series_forecasting_lstm.pth")

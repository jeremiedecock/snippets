...
This snippet is taken from https://fastapi.tiangolo.com/tutorial/bigger-applications/
...
4. Once the container is built and running, you can start the FastAPI server. Run the following command in the terminal inside the container: `fastapi dev app/main.py`
...
3. Run FastAPI in a Podman container. Run the following command in a terminal: `./run.sh` or `podman run --rm -it -v .:/app -w /app -u $(id -u):$(id -g) --userns=keep-id -p 8000:8000 localhost/snippets-fastapi:latest`

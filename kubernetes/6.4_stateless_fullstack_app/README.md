...
- **backend** — the FastAPI API from the previous examples (the plain `1.0`
  image built in `6.1_private_docker_registry`: nothing to rebuild);
...
`2.1_clusterip_service`, every Service gets a stable internal DNS name. That
...
expose both Services through the Gateway of `4.3.1_gateway_api_envoy_gateway`,
with one HTTPRoute rule per path.)

## Build and push the frontend image

From the `frontend/` directory (details in `6.1_private_docker_registry`; same commands
...
Create the namespace for the demo: `kubectl create namespace snippet-fullstack-demo`

Edit `your-username` in both YAML files, then:

`kubectl apply -f backend.yml -f frontend.yml -n snippet-fullstack-demo`

Use it: `kubectl port-forward service/frontend 8080:80 -n snippet-fullstack-demo`,
then open `http://localhost:8080` in a web browser and click the button — the
message `hello` produced by the backend appears on the page.

You can also check the internal chain by hand, from inside the cluster:

```
kubectl run test --rm -it --restart=Never --image=curlimages/curl -n snippet-fullstack-demo -- curl -s http://backend/
kubectl run test --rm -it --restart=Never --image=curlimages/curl -n snippet-fullstack-demo -- curl -s http://frontend/api/
```

Delete everything: `kubectl delete -f backend.yml -f frontend.yml -n snippet-fullstack-demo`

Delete the namespace: `kubectl delete namespace snippet-fullstack-demo`
